...
export default class Repository<T extends IConcept> implements IRepository<T> {
  ...
  constructor(
    collection: Model<T>,
    projectionRule?: ProjectionType<T>,
  ...
  async getAll(
    ...
    project: ProjectionType<T> = this._projectionRule,
    options: QueryOptions<T> = this._queryOptions
  ): Promise<Array<T>> {
    let docs: Array<T> = [];
  ...
  async get(
    ...
    let doc: T = {} as T;
    doc =
      (await this._queryHelper.getDocument(filter, project, options)) ??
      ({} as T);
    return doc;
  }
  ...
  async create(
    ...
    let newDoc: T = {} as T;
    newDoc =
      (await this._queryHelper.createDocument(doc, project, options)) ??
      ({} as T);
    return newDoc;
  }
  ...
  async update(
    filter: FilterQuery<T>,
    update: UpdateAggregationStage | UpdateQuery<T>,
    options: QueryOptions<T> = this._queryOptions
  ): Promise<T> {
    let doc: T = {} as T;
