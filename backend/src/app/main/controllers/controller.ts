...
export default class Controller<T extends IConcept, S extends IService<T>> {
  ...
  public async getDocuments(req: Request, res: Response): Promise<void> {
    let docs: Array<T> = [];
    docs = await this._asyncUtils.wrapFunction(
      this._service.getAll.bind(this._service),
      [req.query]
    );
  ...
  public async getDocument(req: Request, res: Response): Promise<void> {
    let doc: T = {} as T;
    if (req.params)
      doc = await this._asyncUtils.wrapFunction(
        this._service.get.bind(this._service),
        [req.params]
      );
    else
      doc = await this._asyncUtils.wrapFunction(
  ...
  public async createDocument(req: Request, res: Response): Promise<void> {
    let doc: T = {} as T;
    doc = await this._asyncUtils.wrapFunction(
      this._service.create.bind(this._service),
      [req.body]
    );
  ...
  public async deleteDocument(req: Request, res: Response): Promise<void> {
    if (req.params)
      await this._asyncUtils.wrapFunction(
        this._service.delete.bind(this._service),
        [req.params]
      );
    else
      await this._asyncUtils.wrapFunction(
